...
Storybookに表示するストーリーファイルは、`stories`ディレクトリ内に配置します。例えば、コンポーネントの実装ファイルが`src/components/button/sp-button.ts`の場合、対応するストーリーファイルは`stories/button/sp-button.story.ts`となります。

また、このStorybookは、GitHub Pagesを用いて公開しています（[リンク](https://uzabase.github.io/sp-design-components-web-components/)）。コンポーネントを実装する際は、利用者がコンポーネントを使い方を理解できるように、Storybookにストーリーを追加してください。
...
ローカル環境でGitHub Pagesでの表示を確認するには、本番環境用のStorybookをビルドし、ローカルサーバーを立ち上げてください。例えば、以下のコマンドを使用してください。
